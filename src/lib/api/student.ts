import { Student, StudentResponse } from "../type/students";
...
export const getStudentsByParentId = async (parentId: string): Promise<Student[]> => {
  try {
    const response = await fetchData<StudentResponse>(`/students/parent/${parentId}`);
    return response.data || [];
  } catch (error) {
 ...
 */
export const getStudentHealthRecord = async (studentId: string) => {
  try {
    return await fetchData(`/health-records/student/${studentId}`);
  } catch (error) {
