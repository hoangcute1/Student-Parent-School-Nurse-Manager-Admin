 ...
 */
export const createTreatmentHistory = async (data: Partial<TreatmentHistory>) => {
 ...
 * @returns Promise<TreatmentHistory>
 */
export const updateTreatmentHistory = async (id: string, data: Partial<TreatmentHistory>) => {
  const token = getAuthToken();
  console.log("Updating treatment history with ID:", id, "Data:", data);
  console.log("Token to update treatment history:", token);

  const res = await fetch(`http://localhost:3001/treatment-histories/${id}/status`, {
    method: "PUT",
    headers: {
      "Content-Type": "application/json",
      Authorization: `Bearer ${token}`,
...
export const getTreatmentHistoryByParentId = async (parentId: string) => {
  ...
  const res = await fetch(`http://localhost:3001/treatment-histories/parent/${parentId}`, {
    headers: {
      Authorization: `Bearer ${token}`,
      "Content-Type": "application/json"
    },
...
export const getCurrentParentTreatmentHistory = async () => {
  // Import getParentId function
  const { getParentId } = await import("@/lib/utils/parent-utils");

  const parentId = getParentId();
  if (!parentId) {
    throw new Error("Không tìm thấy thông tin phụ huynh hoặc bạn không có quyền truy cập.");
  }
