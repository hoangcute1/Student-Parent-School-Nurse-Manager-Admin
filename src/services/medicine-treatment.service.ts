...
import { MedicineTreatment, MedicineTreatmentDocument } from '@/schemas/medicine-treatment.schema';
...
export class MedicineTreatmentService {
  ...
  async create(createMedicineTreatmentDto: CreateMedicineTreatmentDto): Promise<MedicineTreatment> {
    const createdMedicineTreatment = new this.medicineTreatmentModel(createMedicineTreatmentDto);
    return createdMedicineTreatment.save();
  }

  async findAll(): Promise<MedicineTreatment[]> {
    return this.medicineTreatmentModel.find()
      .populate('treatment')
      .populate('medicine')
      .exec();
  }
  ...
  async findById(id: string): Promise<MedicineTreatment> {
    const medicineTreatment = await this.medicineTreatmentModel.findById(id)
      .populate('treatment')
      .populate('medicine')
      .exec();
  ...
  async findByTreatmentId(treatmentId: string): Promise<MedicineTreatment[]> {
    return this.medicineTreatmentModel.find({ treatment: treatmentId })
      .populate('medicine')
      .exec();
  }

  async update(id: string, updateMedicineTreatmentDto: UpdateMedicineTreatmentDto): Promise<MedicineTreatment> {
  ...
  async remove(id: string): Promise<MedicineTreatment> {
    const deletedMedicineTreatment = await this.medicineTreatmentModel.findByIdAndDelete(id).exec();
