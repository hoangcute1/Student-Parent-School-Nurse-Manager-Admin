...
export class ParentStudentService {
  ...
  async findAll(): Promise<ParentStudent[]> {
    return this.parentStudentModel.find()
      .populate('parent')
      .populate('student')
      .exec();
  }
  ...
  async findById(id: string): Promise<ParentStudent> {
    const parentStudent = await this.parentStudentModel.findById(id)
      .populate('parent')
      .populate('student')
      .exec();
  ...
  async findByParentId(parentId: string): Promise<ParentStudent[]> {
    return this.parentStudentModel.find({ parent: parentId })
      .populate('student')
      .exec();
  }

  async findByStudentId(studentId: string): Promise<ParentStudent[]> {
    return this.parentStudentModel.find({ student: studentId })
      .populate('parent')
      .exec();
  }
