...
import {
  ParentStudent,
  ParentStudentDocument,
} from '@/schemas/parent-student.schema';
...
export class ParentStudentService {
  ...
  async findById(id: string): Promise<ParentStudent> {
    ...
    if (!parentStudent) {
      throw new NotFoundException(
        `Parent-Student relationship with ID ${id} not found`,
      );
    }
  ...
  async remove(id: string): Promise<ParentStudent> {
    const deletedParentStudent = await this.parentStudentModel
