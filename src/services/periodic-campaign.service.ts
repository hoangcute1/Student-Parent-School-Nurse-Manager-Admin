...
import { PeriodicCampaign, PeriodicCampaignDocument } from '@/schemas/periodic-campaign.schema';
...
export class PeriodicCampaignService {
  ...
  async create(createPeriodicCampaignDto: CreatePeriodicCampaignDto): Promise<PeriodicCampaign> {
    const createdPeriodicCampaign = new this.periodicCampaignModel(createPeriodicCampaignDto);
    return createdPeriodicCampaign.save();
  }

  async findAll(): Promise<PeriodicCampaign[]> {
    return this.periodicCampaignModel.find()
      .populate('staff')
      .exec();
  }

  async findById(id: string): Promise<PeriodicCampaign> {
    const periodicCampaign = await this.periodicCampaignModel.findById(id)
      .populate('staff')
      .exec();
  ...
  async update(id: string, updatePeriodicCampaignDto: UpdatePeriodicCampaignDto): Promise<PeriodicCampaign> {
  ...
  async remove(id: string): Promise<PeriodicCampaign> {
    const deletedPeriodicCampaign = await this.periodicCampaignModel.findByIdAndDelete(id).exec();
