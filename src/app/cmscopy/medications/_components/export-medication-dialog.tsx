...
export const ExportMedicationDialog: React.FC<ExportMedicationDialogProps> = ({
  ...
  const [medicalStaffName, setMedicalStaffName] = useState<string>("");
  const [isSubmitting, setIsSubmitting] = useState(false);

  const handleSubmit = async () => {
    if (!medication || !medicalStaffName.trim() || !reason.trim()) {
    ...
    try {
      await onConfirm({
        ...
        quantity,
        reason: reason.trim(),
        medicalStaffName: medicalStaffName.trim(),
      });
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-md bg-white/95 backdrop-blur-sm border-sky-200">
        <DialogHeader>
            ...
            <div className="space-y-4">
              <div>
                ...
                <Input
                  ...
                  className="mt-1 border-sky-200 focus:border-sky-400"
                />
              ...
              <div>
                <Label
                  htmlFor="medicalStaffName"
                  className="text-sm font-medium"
                >
                  Nhân viên y tế <span className="text-red-500">*</span>
                </Label>
                <Input
                  id="medicalStaffName"
                  value={medicalStaffName}
                  onChange={(e) => setMedicalStaffName(e.target.value)}
